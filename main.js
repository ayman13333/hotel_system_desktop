...
const connectDB = require('./my-app/src/db'); // Import the database connection
const _ = require('lodash');



const bcrypt = require('bcrypt');
const Room = require('./my-app/src/Models/Rooms');
const User = require('./my-app/src/Models/Users');
const Book = require('./my-app/src/Models/Books');
...
ipcMain.handle('search-for-room', async (event, data) => {
  try {
    ...
    empty_rooms = rooms?.filter(room => {
      ...
      // عدد الفواتير لازم يكون واحد
      let billsNumber=Booked_rooms.filter(el => el?.roomID.toString() == room?._id.toString());

      console.log('billsNumber',billsNumber.length);

      if (foundRoom?.serialNumber) {
        
        booked_room_count += 1;
        // رجع الغرفة في تاريخ البداية 
        let bookStartDate=new Date(foundRoom?.from);

        if(billsNumber.length<2){
          if(bookStartDate?.getTime()==to?.getTime()){
