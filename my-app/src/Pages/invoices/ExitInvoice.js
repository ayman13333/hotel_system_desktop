...
export default function ExitInvoice() {
    ...
    const handleDownload = async () => {
        ...
        // Save PDF
        pdf.save(`فاتورة-رقم-${location?.state?.serialNumber}-فاتورة-خروج.pdf`);
    ...
    // to=  FormatDate(new Date(location?.state?.to))                                 //      new Date(to).toISOString().split('T')[0];
    let to = new Date(location?.state?.to); 

    
    // if(location?.state?.print||location?.state?.isEdit==false)
    //     {
    //      console.log("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXx",to.setDate(FormatDate(new Date(location?.state?.to))),location?.state?.print );   
            
    //         to=to.setDate(FormatDate(new Date(location?.state?.to)));}
    // else{ to=to.setDate(to.getDate()+1);


    //     console.log("RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR",to.setDate(to.getDate()+1));   
                        ...
                        <div className="invoice-table">
                            <table className="table">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td className='text-center'>  تاريخ  الخروج </td>
                                        <td className='text-center'>{FormatDate( new Date(location?.state?.leaveDate))} </td>
                                        {/* <td className='text-center'>{location?.state?.leaveDate?.toISOString().split('T')[0]} </td> */}
                                    </tr>

                                    <tr>
                                        <td className='text-center'>  تاريخ اصدار الفاتورة </td>
                                        <td className='text-center'>{FormatDate( new Date())} </td>
                                        {/* <td className='text-center'>{new Date(Date.now()).toISOString().split('T')[0]} </td> */}
                                    </tr>
                            ...
                            <table className="table">
                                ...
                                <tbody>
                                    {/* <tr>
                                        ...
                                        <td className='text-center' > {location?.state?.completePrice} L.E </td>
                                    </tr> */}
                                    ...
                                    
                                    {
                                     <tr>
                                        ...
                                        <td className='text-center' style={{fontWeight:'bolder'}} >
                                        {location?.state?.finalPrice} L.E
                                        </td>
                                    </tr> 
                                    }
                                    ...
                                    <tr>
                                        <td className='text-center' > المبلغ المسترد في حالة الخروج مبكرا  </td>
                                        <td className='text-center' > 
                                            {location?.state?.earlyLeavePrice=='' ? 0 : location?.state?.earlyLeavePrice } L.E </td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        المبلغ المسترد من التأمين 
                                        </h4>
                                              </td>
                                        <td className='text-center' >
                                        <h4 style={{ fontWeight: 'bolder' }}>
                                        {Number(location?.state?.ensurancePrice) - Number(location?.state?.subEnsurancePrice)} L.E 
                                         </h4>     
                                        </td>
                                    </tr>
                                    ...
                                    <tr >
                                        ...
                                        <td colSpan={2} className="text-center table-row">
                                          <h4>  {location?.state?.zeroEnsurancePrice ? 
                                        'لم يتم دفع مبلغ تأمين اثناء الحجز' 
                                        :
                                        `تم استرداد مبلغ التأمين `
                                    ...
                                    {/* <tr >
                                        ...
                                        <td colSpan={2} className="text-center table-row">
                                          <h4>  {location?.state?.errorCashBack==0 ? 
                                       ""
                                        :
                                        `يجب ان يرد مبلغ و قدره ${location?.state?.errorCashBack} L.E الي العميل لانك قمت بالتعديل علي اجمالي الفاتةره`
